  ...
  return (
    <div className="card p-4 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-muted font-mono">{fmt(total_days - used_days)} d left</div>
      </div>
      <div className="text-2xl font-semibold text-heading font-mono">
        {fmt(used_days)}{" "}
        <span className="text-muted text-sm font-normal">
          / {fmt(available_days)}
          {category.accrues && available_days !== total_days && <> avail / {fmt(total_days)}</>} d
        </span>
      </div>
      <div className="relative h-2 rounded-full bg-[color:var(--color-hover)] overflow-hidden">
        {category.accrues && (
          <div
            className="absolute top-0 bottom-0 w-px bg-[color:var(--color-body)] opacity-60"
            style={{ left: `${availPct}%` }}
      ...
      {over_accrual_days > 0 && (
        <div className="flex items-start gap-1 text-xs text-[color:var(--color-warning,#b45309)]">
          ...
          <span>
            Borrowing {fmt(over_accrual_days)} d from later in the year — make sure you stick around
            to earn it back.
          </span>
        </div>
      )}
...
function fmt(n: number): string {
  return n.toFixed(2).replace(/0+$/, "").replace(/\.$/, "").replace(/^-?0$/, "0");
}

...
export function renderPrintHTML(data: PrintData): string {
  ...
  const css = `
    /* Page geometry: Puppeteer page.pdf() sets margins; @page is the
       fallback for ad-hoc Cmd+P print or future format changes. */
    @page { size: A4 portrait; margin: 18mm; }
    /* Force light color scheme — Puppeteer in headless mode can otherwise
       inherit the system's dark scheme defaults for form widgets. */
    html { color-scheme: light; background: white; }
    * { box-sizing: border-box; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Liberation Sans", "DejaVu Sans", Helvetica, Arial, sans-serif; color: #111827; margin: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    h2 { font-size: 18px; margin: 24px 0 8px 0; border-bottom: 1px solid #e5e7eb; padding-bottom: 4px; page-break-after: avoid; break-after: avoid; }
    h3 { page-break-after: avoid; break-after: avoid; }
    table { width: 100%; border-collapse: collapse; font-size: 12px; }
    th, td { padding: 6px 8px; text-align: left; border-bottom: 1px solid #f1f5f9; word-break: break-word; }
    th { background: #f9fafb; font-weight: 600; color: #374151; }
    /* Repeat the header row on every page break for multi-page tables. */
    thead { display: table-header-group; }
    tr { page-break-inside: avoid; break-inside: avoid; }
    /* Keep each month's table together when the page break would otherwise
       split a single row off from its header. */
    .month { margin-top: 16px; page-break-inside: avoid; break-inside: avoid; }
    .pill { display: inline-block; padding: 2px 8px; border-radius: 999px; color: white; font-size: 10px; font-weight: 600; letter-spacing: 0.02em; max-width: 200px; overflow-wrap: break-word; }
    .num { font-variant-numeric: tabular-nums; }
    .totals td { font-weight: 600; }
    .footer { margin-top: 48px; font-size: 10px; color: #6b7280; }
    .legend { font-size: 10px; color: #6b7280; margin-top: 4px; }
    .desc { color: #4b5563; }
    .cancelled td { text-decoration: line-through; color: #9ca3af; }
    /* Signature block for HR submission flows. */
    .signatures { margin-top: 64px; display: flex; gap: 48px; page-break-inside: avoid; break-inside: avoid; }
    .signatures div { flex: 1; border-top: 1px solid #9ca3af; padding-top: 4px; font-size: 10px; color: #6b7280; }
  `;
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="color-scheme" content="light"><title>${escapeHtml(user.display_name)} — AFK ${year}</title>
...
<body>
  ...
  <h2>Vacations</h2>
  <div class="legend">Strikethrough rows = cancelled.</div>

...
/**
 * Reject obviously bogus year values before they hit D1. The path regex
 * already rejects non-digits, but Number("9999999999999999") yields a
 * non-integer; bound it to a sane window.
 */
...
r.get("/allowances/:year{[0-9]+}", async (c) => {
  ...
  if (!validYear(year)) return err(c, "VALIDATION_ERROR", "Year out of range.");
...
r.put("/allowances/:year{[0-9]+}/:categoryId", async (c) => {
  ...
  const body =
    (await c.req.json<AllowanceBody>().catch(() => ({}) as AllowanceBody)) ?? ({} as AllowanceBody);
  ...
  const notes =
    body.notes == null
      ? null
      : // eslint-disable-next-line no-control-regex
        body.notes.replace(/[\x00-\x1F\x7F]+/g, " ").slice(0, 500);
  try {
